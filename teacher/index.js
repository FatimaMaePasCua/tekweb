...
app.get('/archivedClasses',function(req, res){
	var userID = req.session.userID;

	if(userID){
		var sql = "Select * from classes where userID = ? AND status = 'archived'";

		connection.query(sql,[userID], function (err, result) {	
			if (err) throw err;
			res.render('archives',{classes: result});
		});
	}else{
		res.redirect('/logout')
	}
...
app.get('/grades/:classID',function(req, res){
	...
	if(userID){
		...
		var sql2 = `Select * from classes where classID = ?`;
		...
		connection.query(sql2,[classID], function (err, result) {
			...
			connection.query(sql,[classID], function (err, result) {
				if (err) throw err;
				res.render('grades',{grades: result,classC : classC});
			});
		});
	}else{
		res.redirect('/logout')
...
app.get('/announcements',function(req, res){
	...
	if(userID){
		...
		WHERE classes.status='active' AND classes.userID = ?`;
...
app.post('/archive',function(req, res){
	...
	var sql = "Update classes set status='archived' where classID =?";
	...
	connection.query(sql,[classID], function (err, result) {
		...
		connection.query(sql2,[classID], function (err, result) {
			if (err) throw err;
			var classes = result[0];
			var sql2 = "Insert into transactions (action,userID,classID) values('Class "+classes.classCode+" has been archived.',?,?)";
			connection.query(sql2,[userID,classID], function (err, result) {
				if (err) throw err;
				res.redirect('/classes');
			});
...
app.post('/revert',function(req, res){
	...
	var sql = "Update classes set status='active' where classID =?";
	...
	connection.query(sql,[classID], function (err, result) {
		...
		connection.query(sql2,[classID], function (err, result) {
			if (err) throw err;
			var classes = result[0];
			var sql2 = "Insert into transactions (action,userID,classID) values('Class "+classes.classCode+" has been reverted.',?,?)";
			connection.query(sql2,[userID,classID], function (err, result) {
				if (err) throw err;
				res.redirect('/archivedClasses');
			});
		});
	});
});
...
app.post('/uploadGrade/:classID/:studentID', function(req, res) {
	...
	if(req.body.prelims && req.body.midterms && req.body.finals){
		var finalGrade = (parseInt(prelims) + parseInt(midterms) + parseInt(finals))/3;
	}
	var sql = "Update grades set Preliminary = ?, Midterms = ?, Finals = ?,FinalGrade = ? where classID =? AND studentID =?";


	connection.query(sql,[prelims,midterms,finals,finalGrade,classID,studentID], function (err, result) {		
		if (err) throw err;
